...
/**
 * This class provides a string that sometimes seems to change value, as far as
 * equality tests and <code>startswith</code> are concerned. This solves a
 * problem that Jython users sometimes experience with libraries that are
 * sensitive to platform.
 * <p>
 * A library may test for a particular platform in order to adjust to local file
 * name conventions or to decide which operating system commands are available.
 * In Jython, <code>os.name</code> and <code>sys.platform</code> indicate that
 * Java is the platform, which is necessary information in some parts of the
 * standard library, but other libraries assuming CPython then draw incorrect
 * conclusions.
 * <p>
 * With hindsight, a better choice could be made, where
 * <code>sys.platform</code> etc. indicated Windows or Posix, and something else
 * indicates the implementation. A change in Jython 2 would cause more problems
 * than it solved, but we expect Jython 3 to work that way. In the Python
 * Standard Library, the Jython project can make all necessary changes. It can't
 * do anything about third-party libraries. But it would be a big help if users
 * could cause <code>sys.platform</code> or <code>os.name</code> to appear to
 * have the OS-dependent value as far as those troublesome libraries were
 * concerned.
 * <p>
 * This is what this class achieves. <code>os.name</code> and
 * <code>sys.platform</code> regular strings for most purposes, but each has a
 * "shadow" value that is used in contexts the user may specify.
 */
@Untraversable
...
public class PyShadowString extends PyString {
	...
	/**
	 * Contexts (expressed as a {@link PyTuple} of class name and method name) where
	 * {@link #shadow} is allowed to match as well as the primary value.
	 */
	protected PyList targets;

	/**
	 * The shadow string is additionally used for some comparisons, especially for
	 * __eq__. __eq__ will evaluate positive if the other string equals the primary
	 * value *or* the shadow. The shadow persists slicing (is sliced accordingly)
	...
	/**
	 * Construct an instance specifying primary and shadow values (bytes objects
	 * expected).
	 */
	...
	static PyObject shadowstr_new(PyNewWrapper new_, boolean init, PyType subtype, PyObject[] args, String[] keywords) {
		ArgParser ap = new ArgParser("shadowstr", args, keywords, new String[] { "primary", "shadow" }, 0);
		...
		if (valueObj instanceof PyString && shadowObj instanceof PyString) {
			...
			String message = String.format("arguments must be strings not (%.200s, %.200s)", valueObj.getType(),
					shadowObj.getType());
	...
	/**
	 * Test whether the current code is executing in of one of the target contexts,
	 * by searching up the stack for a class and method pait that match.
	 *
	...
	/**
	 * Specify a context (class, method) in which the shadow string is allowed to
	 * match.
	 *
	 * @param className
	...
	/**
	 * Return a list of the tuples specifying the contexts in which the shadow value
	 * will be consulted during matching.
	 */
	...
	/**
	 * Compare this <code>PyShadowString</code> with another <code>PyObject</code>
	 * for equality. A <code>PyShadowString</code> is equal to the other object if
	 * its primary value is equal to it, or if its shadow value is equal to the
	 * other object and the test is made in one of its target contexts. (Two
	 * <code>PyShadowString</code> are equal if the primary values are equal, the
	 * primary of one matches the shadow of the other in the shadow's context, or
	 * their shadows match and both are in context.
	 *
	 * @param other
	 *            to compare
	 * @return <code>PyBoolean</code> result (or <code>null</code> if not
	 *         implemented)
	 */
	@Override
	...
	final PyObject shadowstr___eq__(PyObject other) {
		// Re-wrap the primary value as a PyString to invoke the right kind of equality
		// test.
	...
	/**
	 * Test for equality, used as a helper to <code>shadowstr___eq__</code>, dealing
	 * with the possibility that <code>other</code> is another
	 * <code>PyShadowString</code>.
	 */
	...
	final boolean shadowstr_startswith(PyObject prefix, PyObject startObj, PyObject endObj) {
		return super.startswith(prefix, startObj, endObj) //
				|| (!targets.isEmpty() && shadow.startswith(prefix, startObj, endObj) && isTarget());
	}
